...
export function SleepSummaryCard() {
  ...
  const { current, accentColor } = getActionRowProgressDisplay("sleep");
  const barFillPercent = getActionRowFillPercent("sleep");
  const progressPercent = getActionRowProgressPercent("sleep");
  const progressPercentLabel = `${Math.round(progressPercent)}%`;
  const progressRest = sleepRow.progressRest;
